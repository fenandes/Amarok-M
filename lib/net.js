...
module.exports = async (Amarok, m, commands, chatUpdate, store) => {
  try {
    ...
    if (
      ...
      if (m.isGroup && !isCreator && modStatus == "false") {
        return m.reply(
          `*${botName}* is *Banned* on *${groupName}* group! \n\nType *${prefix}owner* or *${prefix}support* to submit a request to unban the group!`
        );
      }
    }
    ...
    if (body.startsWith(prefix) && !icmd) {
      let amaroktext = `No such command *${pushname}* Type *${prefix}menu* to get my full command list!\n`;
      const reactmxv = {
        react: {
          text: '🚫',
          key: m.key,
      ...
      Amarok.sendMessage(m.from, {image: {url : "https://i.imgur.com/ZgrSw7W.jpg"}, caption: amaroktext}, {
        quoted: m,
      });
    }
    ...
    if(!isCreator&&modStatus == "false"&&!botNumber.includes(m.sender)){
